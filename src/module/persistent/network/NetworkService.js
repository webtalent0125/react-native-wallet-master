...
async function getActiveNetwork() {
  const activeNetwork =
    (await LMStorageService.getItem(LMStorageConstant.ACTIVE_NETWORK_STORAGE_KEY)) ||
    ApplicationProperties.ACTIVE_NETWORK;
  return activeNetwork;
}

async function list() {
  await LMStorageService.deleteItem(LMStorageConstant.NETWORKS_STORAGE_KEY);
  let networks =
    (await LMStorageService.getItem(LMStorageConstant.NETWORKS_STORAGE_KEY)) ||
    ApplicationProperties.NETWORKS;
