...
const AddWatchList = (props) => {
  ...
  const addNewToken = () => {
    LMLoading.show();
    dispatch(TokenAction.addCustomTokens(newAsset));
    dispatch(TokenAction.addToWatchList(newAsset.symbol, newAsset.slug));
    LMLoading.hide();
    resetForm();
                ...
                if (quotes[token.symbol])
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => addToWatchList(token.symbol, token.slug)}
                    >
                      <Image
                        source={{
                          uri:
